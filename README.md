...
- **RAM saver** (experimental, opt-in under Settings → Advanced): downloads stage out-of-order chunks to temp files and re-read them in order, instead of holding decrypted plaintext in an in-memory reorder buffer. This keeps peak memory flat as the per-transfer slot count and number of concurrent downloads climb, at the cost of roughly doubling download disk writes — so it's worth enabling only at high concurrency. Off by default; output is byte-identical either way.
